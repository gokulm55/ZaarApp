...
@objc open class ACFloatingTextfield: UITextField {
    ...
    @IBInspectable open var selectedLineColor : UIColor = UIColor.systemBlue{
        didSet{
            self.floatTheLabel()
        }
    }
    ...
    @IBInspectable open var selectedPlaceHolderColor : UIColor = UIColor.white{
        didSet{
            self.floatTheLabel()
        }
    }
...
fileprivate extension ACFloatingTextfield {
    ...
    func addFloatingLabel(){
        ...
        labelPlaceholder?.font = UIFont.init(name: (self.font?.fontName ?? "helvetica")!, size: 15)
